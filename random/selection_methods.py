...
def query_samples(model, method, data_unlabeled, subset, labeled_set, cycle, args):
    ...
    if method == 'MinCut':
        ...
        # Sort the eigenvalues in ascending order and select the first k
        k = ADDENDUM
        eigenvalues_sorted_idx = torch.argsort(eigenvalues)
        eigenvectors_selected = eigenvectors[:, eigenvalues_sorted_idx[:k]]

        # Perform spectral clustering to divide the nodes into different clusters
        n_clusters = 2
        sc = SpectralClustering(n_clusters=n_clusters, affinity='nearest_neighbors', n_neighbors=10)
        sc.fit(eigenvectors_selected.cpu().numpy())

        # 选取标记数据所属的群集，然后选择另一个群集中最不相似的k个数据点
        labeled_cluster = sc.labels_[:ADDENDUM]
        unlabeled_cluster = sc.labels_[ADDENDUM:]

        # Find the nodes that belong to a different cluster than the labeled data
        unlabeled_indices = torch.nonzero(torch.tensor(unlabeled_cluster != labeled_cluster), as_tuple=True)[0]

        # Calculate the sum of distances between each unlabeled data point and the labeled data points, then choose the k smallest ones
        distances = torch.cdist(labeled_features, unlabeled_features, p=2).sum(dim=0)
        most_unsimilar_indices = torch.argsort(distances)[:ADDENDUM]

        # Select the indices of the new labeled samples and add them to labeled_set
        arg = most_unsimilar_indices.cpu()
